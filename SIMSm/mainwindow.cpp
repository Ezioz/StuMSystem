...
void MainWindow::display_course_info()
{
    ...
    while(p != NULL)
    {
        ...
        ui->tableWidget_2->setItem(rows, 2, new QTableWidgetItem((p->type == 0)?"4:6":((p->type == 1)?"2:8":"1.5:8.5")));
        p = p->next;
    }
}
...
void MainWindow::display_choose_info()
{
    ...
    } for(int loop = 0; loop < ui->tableWidget_4->rowCount();){
        ui->tableWidget_4->removeRow(loop);
    }
    ...
    while(p != NULL)
    {
        ...
        int rows1 = ui->tableWidget_4->rowCount();
        ...
        ui->tableWidget_3->setItem(rows,4,new QTableWidgetItem(((p->regu_score)==-1)?"":QString::number(p->regu_score)));
        if(pcs->type==0)
        ui->tableWidget_3->setItem(rows, 5, new QTableWidgetItem(((p->grade)==-1)?"":QString::number(((p->grade)*5-(p->regu_score)*2)/3)));
        if(pcs->type==1)
            ui->tableWidget_3->setItem(rows, 5, new QTableWidgetItem(((p->grade)==-1)?"":QString::number(((p->grade)*5-p->regu_score)/4)));
        if(pcs->type==2)
            ui->tableWidget_3->setItem(rows,5,  new QTableWidgetItem(((p->grade)==-1)?"":QString::number(((p->grade)-(p->regu_score)*0.15)/0.85)));
        ui->tableWidget_3->setItem(rows, 6, new QTableWidgetItem(((p->grade)==-1)?"":QString::number(p->grade)));
            if((p->grade)<60){
            ui->tableWidget_4->insertRow(rows1);
            ui->tableWidget_4->setItem(rows1, 0, new QTableWidgetItem(p->stu));
            ui->tableWidget_4->setItem(rows1, 1, new QTableWidgetItem(name_tmp));
            ui->tableWidget_4->setItem(rows1, 2, new QTableWidgetItem(p->crs));
            ui->tableWidget_4->setItem(rows1, 3, new QTableWidgetItem(QString::number(score_tmp)));

            ui->tableWidget_4->setItem(rows1, 4, new QTableWidgetItem(((p->grade)==-1)?"":QString::number(p->grade)));
}
        p = p->next;
    }
}
...
void MainWindow::on_class_add_triggered()
{
    ...
    QString text3 = QInputDialog::getText(NULL, tr("添加成绩占比"), tr("1代表2:8，0代表4:6"));
    if(text3 == NULL)
        return;
    QByteArray bytearray3 = text3.toLocal8Bit();
    char *scoredivide = bytearray3.data();

    CourseInfo *p = new CourseInfo;
    p->next = NULL;
    p->score = atoi(coursescore);
    p->type=atoi(scoredivide);
...
void MainWindow::on_stushow_clicked()
{
    ui->tabWidget->setCurrentIndex(0);
}


void MainWindow::on_stushow_2_clicked()
{
     ui->tabWidget->setCurrentIndex(1);
}

void MainWindow::on_stushow_3_clicked()
{
     ui->tabWidget->setCurrentIndex(2);
}
void MainWindow::on_stushow_4_clicked()
{
    ui->tabWidget->setCurrentIndex(3);
}

void MainWindow::on_mainwindowclose_clicked()
{
    LoginDialog a;
    a.exec();
}
